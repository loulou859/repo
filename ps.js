...
var PURSTREAM_REFERER = PURSTREAM_REFERER_DEFAULT;
...
var PURSTREAM_REDIRECT = 'https://purstream.wiki/';
var PURSTREAM_TELEGRAM = 'https://t.me/s/purstreamm';

// ---------------------------------------------------------------
// Détection du domaine actif
// 1er choix : purstream.wiki
...
function applyPurstreamDomain(base) {
  var tld = base.replace(/https?:\/\/purstream\./, '');
  PURSTREAM_API     = 'https://api.purstream.' + tld + '/api/v1';
  PURSTREAM_REFERER = base + '/';
}
...
function getStreams(tmdbId, mediaType, season, episode) {
  ...
  return pipeline()
    .catch(function(err) {
      console.warn('[Purstream] Échec domaine principal (' + err.message + '), détection nouveau domaine...');
      return detectPurstreamDomain().then(function(newBase) {
        if (!newBase) throw new Error('Aucun domaine trouvé');
        applyPurstreamDomain(newBase);
        return pipeline();
      });
    })
    .catch(function(err) {
